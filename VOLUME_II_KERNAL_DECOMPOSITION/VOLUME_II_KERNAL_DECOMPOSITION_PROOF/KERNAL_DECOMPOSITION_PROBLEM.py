...
T1-level kernel algebra, curvature/floor decomposition, Fourier symbol,
Bochner-PSD checks, and numerical probes for the sech^2 / sech^4 kernel.

Core identities (AIREADME dictionary):
...
All statements here are T1-unconditional: finite algebra/calculus only.
"""
...
# High precision for analytic checks and tight TDD tolerances
mp.mp.dps = 80

# ---------------------------------------------------------------------------
# 1. Hyperbolic primitives
# ---------------------------------------------------------------------------
...
def w_H_double_prime(t: float | mp.mpf, H: float | mp.mpf) -> mp.mpf:
    ...
    Equivalent to the AIREADME form W_curv(t) = -w_H''(t)
    = (2/H^2)(1 - 3 tanh^2(u)) sech^2(u).
    """
    u = t / H
    s2 = sech2(u)
    th = tanh_(u)
...
def k_H(t: float | mp.mpf, H: float | mp.mpf) -> mp.mpf:
    ...
    """
    H2 = H * H
    lam_star = mp.mpf(4) / H2
    return -w_H_double_prime(t, H) + floor_term(t, H, lam_star)
...
# ---------------------------------------------------------------------------
# 3. Sign structure of w_H'' and curvature term
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. Minimal λ* = 4/H^2 and λ-sharpness
# ---------------------------------------------------------------------------
...
def k_H_L2_squared(H: float | mp.mpf) -> mp.mpf:
    ...
    Exact rational arithmetic using mpmath.
    """
    H = mp.mpf(H)
...
# ---------------------------------------------------------------------------
# 8. λ-perturbation negativity search
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 9. Diagnostic runner (optional)
# ---------------------------------------------------------------------------

def demo_volume_ii_diagnostics():
    H = 1.0
    print("=== Volume II: Kernel Decomposition Diagnostics (H=1) ===")
    ...
    print(f"Max identity error k_H vs sech^4 closed form: {max_err:.3e}")

    info = w_double_prime_sign_info(H)
    print("Transition u =", info["transition_u"])
    print("Transition t =", info["transition_t"])
    a, b = curvature_negative_interval(H)
    print("Curvature negative interval (t):", (a, b))

    lam_num = minimal_lambda_numeric(H=H, t_max=5.0, n_grid=8001)
    print(f"Numeric minimal λ on [-5,5]: {lam_num:.6f}, λ* = {lambda_star(H):.6f}")

    ok_hat = fourier_symbol_nonnegative(H=H, xi_max=10.0, n_grid=4001)
    print("Fourier symbol nonnegative on grid?", ok_hat)

    ok_psd = bochner_psd_check(N=20, H=H)
    print("Toeplitz Gram PSD (N=20)?", ok_psd)

    print("∫ k_H =", k_H_L1(H))
    print("∫ k_H^2 =", k_H_L2_squared(H))
    print("Decay samples:")
    for t, val in k_H_decay_sample(H=H):
        print(f"  t={t:5.2f}, k_H(t)={val:.3e}")

    lam_star_val, t_neg, val_neg = lambda_sharpness_verbose(H=H)
    print(f"Lambda sharpness check (H=1): λ*={lam_star_val}, t_neg={t_neg}, k_λ(t_neg)={val_neg}")
